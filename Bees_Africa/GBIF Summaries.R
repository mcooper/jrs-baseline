...
setwd('D://Documents and Settings/mcooper/Google Drive/JRS/Bees_Africa/')
...
fields <- c('name', 'key', 'decimalLatitude', 'decimalLongitude',
            ...
            'year', 'month', "datasetName", "country", 'genus',
            'institutionCode', 'eventDate', 'dateIdentified',
            'basisOfRecord')

res <- occ_search(out$familyKey, continent=continent, fields=fields,
                  limit=200000)

dat <- res$data

write.csv(dat, paste0(taxa, '_', continent, '.csv'), row.names=F)
...
datasets$DatasetName[datasets$DatasetName == "List of animalia, fungi and plant species recorded through naturalist observations and research activities in Benin. Data mobilized in the framework of a JRS Biodiversity Foundation project of Benin"] <- "JRS Biodiversity Foundation project of Benin"
...
ggplot(dat, aes(genus)) + geom_bar() + theme_bw() + 
  ...
  #scale_fill_manual(values=dat$fill) + 
  #guides(fill=guide_legend(title="Dataset Name")) + 
  coord_flip() + 
