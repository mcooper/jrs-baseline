setwd('D://Documents and Settings/mcooper/Google Drive/JRS/Lepidoptera-EastAfrica/')
...
dat <- bind_rows(res$KE$data, res$TZ$data, res$UG$data)
...
ggplot(dat) + geom_bar(aes(x='', fill=countryName), width=1) +
  ...
  theme(axis.text = element_blank(),
        ...
        panel.grid  = element_blank()) +
  xlab('') + ylab('') +
  scale_fill_manual(values=c(Kenya='#7D2D2E', `Tanzania, United Republic of`='#6C832E', Uganda='#00727B')) +
...
mkTable(getTopX(as.character(dat$family), 20))
...
ggplot(dat) + geom_histogram(aes(x=year), binwidth=1) + 
  theme_bw() +
  theme(legend.position="bottom",legend.direction="vertical") +
  scale_x_continuous(limits=c(1850, 2018), expand = c(0, 0))# + 
#guides(fill=guide_legend(title="Dataset Name")) + 
#scale_fill_manual(values=levels(dat$DatasetNameCol))
